...
function addDependencies(buildGradle) {
  ...
  var fabricDependency = whitespace + 'classpath \'io.fabric.tools:gradle:1.25.4\' // fabric dependency from fcm plugin'
...
function addRepos(buildGradle) {
  ...
  var fabricMavenRepo = whitespace + 'maven { url \'https://maven.fabric.io/public\' } // Fabrics Maven repository from fcm plugin'
  var modifiedLine = match[0] + '\n' + fabricMavenRepo;
  ...
  if (allProjectsIndex > 0) {
    ...
    var googlesMavenRepo = whitespace + 'google() // Google\'s Maven repository from fcm plugin';
